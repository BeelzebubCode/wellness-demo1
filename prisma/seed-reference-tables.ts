/**
 * Seed script สำหรับ Reference Tables
 * Populate ข้อมูลจาก enum เดิมลง reference tables ใหม่
 *
 * วิธีรัน: npx ts-node prisma/seed-reference-tables.ts
 */
...
async function seedGenderCategory() {
  const data = [
  ...
  for (const item of data) {
    await prisma.genderCategory.upsert({
      where: { code: item.code },
      update: item,
      create: item,
    });
  }
  console.log("✅ GenderCategory seeded");
}

async function seedBloodGroupCategory() {
  const data = [
  ...
  for (const item of data) {
    await prisma.bloodGroupCategory.upsert({
      where: { code: item.code },
      update: item,
      create: item,
    });
  }
  console.log("✅ BloodGroupCategory seeded");
}

async function seedIncomeBracketCategory() {
  const data = [
  ...
  for (const item of data) {
    await prisma.incomeBracketCategory.upsert({
      where: { code: item.code },
      update: item,
      create: item,
    });
  }
  console.log("✅ IncomeBracketCategory seeded");
}

async function seedParentalStatusCategory() {
  const data = [
  ...
  for (const item of data) {
    await prisma.parentalStatusCategory.upsert({
      where: { code: item.code },
      update: item,
      create: item,
    });
  }
  console.log("✅ ParentalStatusCategory seeded");
}

async function seedEducationLevelCategory() {
  const data = [
  ...
  for (const item of data) {
    await prisma.educationLevelCategory.upsert({
      where: { code: item.code },
      update: item,
      create: item,
    });
  }
  console.log("✅ EducationLevelCategory seeded");
}

async function seedAddressTypeCategory() {
  const data = [
  ...
  for (const item of data) {
    await prisma.addressTypeCategory.upsert({
      where: { code: item.code },
      update: item,
      create: item,
    });
  }
  console.log("✅ AddressTypeCategory seeded");
}

async function seedNationalityTypeCategory() {
  const data = [
  ...
  for (const item of data) {
    await prisma.nationalityTypeCategory.upsert({
      where: { code: item.code },
      update: item,
      create: item,
    });
  }
  console.log("✅ NationalityTypeCategory seeded");
}

async function seedAcademicPeriodTypeCategory() {
  const data = [
  ...
  console.log("✅ AcademicPeriodTypeCategory seeded");
}

async function seedServiceModeCategory() {
  const data = [
    { code: "ONSITE", name_th: "พบหน้า (Onsite)", name_en: "Onsite", sort_order: 1 },
    { code: "ONLINE", name_th: "ออนไลน์", name_en: "Online", sort_order: 2 },
  ];
  for (const item of data) {
    await prisma.serviceModeCategory.upsert({
      where: { code: item.code },
      update: item,
      create: item,
    });
  }
  console.log("✅ ServiceModeCategory seeded");
}

async function seedUniversityTypeCategory() {
  const data = [
  ...
  for (const item of data) {
    await prisma.universityTypeCategory.upsert({
      where: { code: item.code },
      update: item,
      create: item,
    });
  }
  console.log("✅ UniversityTypeCategory seeded");
}

async function main() {
  console.log("🌱 Seeding reference tables...\n");

  await seedGenderCategory();
  await seedBloodGroupCategory();
  await seedIncomeBracketCategory();
  await seedParentalStatusCategory();
  await seedEducationLevelCategory();
  await seedAddressTypeCategory();
  await seedNationalityTypeCategory();
  await seedAcademicPeriodTypeCategory();
  await seedServiceModeCategory();
  await seedUniversityTypeCategory();

  console.log("\n✅ All reference tables seeded successfully!");
}
