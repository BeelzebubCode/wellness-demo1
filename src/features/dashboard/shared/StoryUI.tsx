        ...
        <div className="flex flex-wrap items-center gap-1.5">
            {DATE_PRESETS.map(p => (
                <button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-bold transition-all duration-200 ${value === p.value
            ...
            {showCustom && value === "custom" && (
                <div className="flex items-center gap-1.5 ml-1">
                    <input
                        ...
                        className="px-2 py-1 rounded-lg border border-slate-200 text-xs text-slate-600 bg-white focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 outline-none transition-all"
                    />
    ...
    return (
        <div className="flex bg-slate-100 rounded-lg p-0.5 shrink-0">
            <button onClick={() => onChange("count")}
                className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-bold transition-all ${value === "count" ? "bg-white text-slate-700 shadow-sm" : "text-slate-400"
                    }`}>
                <Hash className="w-3.5 h-3.5" /> จำนวน
            </button>
            <button onClick={() => onChange("percent")}
                className={`flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-bold transition-all ${value === "percent" ? "bg-white text-slate-700 shadow-sm" : "text-slate-400"
                    }`}>
                <Percent className="w-3.5 h-3.5" /> เปอร์เซ็นต์
            </button>
        </div>
    );
}
