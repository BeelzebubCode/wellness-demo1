// src/features/dashboard/rector/components/RectorInsightPanel.tsx
"use client";
...
import {
    ...
    DollarSign, BarChart3, Minus, AlertTriangle
} from "lucide-react";
...
// ── Fac card ───────────────────────────────────────────────────────────────
function FacultyBudgetCard({ fac, rank, signal, avg, topProblem }: {
    fac: FacultySummary; rank: number; signal: BudgetSignal; avg: number; topProblem?: string;
}) {
    const meta = BUDGET_META[signal];
    const BudgetIcon = meta.icon;
    const pct = avg > 0 ? Math.round((fac.count / avg - 1) * 100) : 0;
    ...
    return (
        ...
        >
            ...
            <div className="flex items-start gap-2 pr-8">
                ...
                <p className="text-sm font-bold text-slate-700 leading-snug line-clamp-2">{fac.nameTh}</p>
            </div>

            <div className="flex items-baseline gap-1.5">
                <span className={`text-2xl font-black ${meta.color}`}>{fac.count.toLocaleString()}</span>
...
// ── Main Component ─────────────────────────────────────────────────────────
export function RectorInsightPanel() {
    ...
    const fetchData = useCallback(async (p: Preset) => {
        ...
        try {
            ...
            // Rector story API returns everything in 'all' (including facultyBookings)
            const [allRes, riskRes] = await Promise.all([
            ...
            const facs: FacultySummary[] = [...(d.facultyBookings ?? [])].sort((a: any, b: any) => b.count - a.count);
    ...
    // Compute avg, signals
    const facAnalysis = useMemo(() => {
        if (!insight) return [];
        const avg = insight.faculties.length > 0
            ? insight.faculties.reduce((s, d) => s + d.count, 0) / insight.faculties.length
            : 0;
        return insight.faculties.map((fac, i) => ({
            fac,
            rank: i + 1,
            signal: getBudgetSignal(fac.count, avg),
            avg,
            topProblem: insight.topProblems[0]?.label,
        }));
    }, [insight]);

    const visibleFacs = showAll ? facAnalysis : facAnalysis.slice(0, 4);

    return (
        <div className="relative overflow-hidden rounded-[2.5rem] bg-white border border-purple-100 p-7 mb-2"
            style={{ boxShadow: "0 4px 32px -4px rgba(168,85,247,0.08)" }}>
            {/* Decorative orbs */}
            <div className="absolute -top-16 -left-16 w-48 h-48 rounded-full bg-purple-200/20 blur-3xl pointer-events-none" />
            <div className="absolute -bottom-16 -right-16 w-48 h-48 rounded-full bg-indigo-100/30 blur-3xl pointer-events-none" />

            {/* ── Header ── */}
            <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-3">
                    <div className="p-2.5 rounded-2xl bg-gradient-to-br from-purple-500 to-indigo-600 shadow-md">
                        <Sparkles className="w-5 h-5 text-white" />
                    </div>
                    <div>
                        <h3 className="text-lg font-black text-slate-800 tracking-tight flex items-center gap-2">
                            สรุปข้อมูลระดับมหาวิทยาลัยและข้อเสนอแนะเชิงนโยบาย
                            <span className="text-[10px] font-bold bg-gradient-to-r from-purple-500 to-indigo-500 text-white px-2 py-0.5 rounded-full uppercase tracking-widest">
                                Auto Analysis
                            </span>
                        </h3>
                    ...
                    <div className="flex items-center p-1 rounded-xl bg-slate-100/80 border border-slate-200/50 shadow-inner">
                        {PRESETS.map(p => (
                            <button key={p.value} onClick={() => setPreset(p.value)}
                                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-200 ${preset === p.value ? "bg-white text-purple-600 shadow-sm border border-purple-100" : "text-slate-500 hover:text-slate-700"}`}>
                                {p.label}
                            </button>
                        ))}
                    </div>
            ...
            <AnimatePresence mode="wait">
                {loading ? (
                    ...
                    <motion.div key="data" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                        ...
                        {/* ── Faculties Budget Analysis ── */}
                        {facAnalysis.length > 0 && (
                            <div>
                                <div className="flex items-center gap-2 mb-3">
                                    <DollarSign className="w-4 h-4 text-purple-500" />
                                    <h4 className="text-sm font-black text-slate-700">การจัดสรรงบประมาณกระจายตามคณะ</h4>
                                    <span className="text-[10px] text-slate-400 font-medium">— เปรียบเทียบจากค่าเฉลี่ยมหาวิทยาลัย</span>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
                                    {visibleFacs.map(({ fac, rank, signal, avg, topProblem }) => (
                                        <FacultyBudgetCard key={fac.id} fac={fac} rank={rank} signal={signal} avg={avg} topProblem={topProblem} />
                                    ))}
                                </div>
                                {facAnalysis.length > 4 && (
                                    <button onClick={() => setShowAll(v => !v)}
                                        className="mt-3 flex items-center gap-1.5 text-xs font-bold text-purple-500 hover:text-purple-700 transition-colors">
                                        <BarChart3 className="w-4 h-4" />
                                        {showAll ? "ย่อ" : `ดูทั้งหมด ${facAnalysis.length} คณะ`}
                                    </button>
                                )}
                            </div>
                        )}
                        ...
                        <div className="flex flex-wrap gap-2 items-center pt-2 border-t border-slate-100">
                            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">ภาพรวมมหาวิทยาลัย:</span>
                            ...
                            {facAnalysis.filter(d => d.signal === "increase").length > 0 && (
                                <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-rose-50 text-rose-600 border border-rose-100">
                                    ...
                                    เพิ่มงบ {facAnalysis.filter(d => d.signal === "increase").length} คณะ
                                </span>
                            )}
                        </div>
                    </motion.div>
