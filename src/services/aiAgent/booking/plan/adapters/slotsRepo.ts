// src/services/aiAgent/booking/plan/adapters/slotsRepo.ts
import prisma from "@/lib/prisma";
import { bkkRange, toMinBkk, bkkTodayISO } from "../utils/time";
...
export async function listAvailableSlots(params: {
  ...
  const filtered = minMin == null ? mapped : mapped.filter((s) => toMinBkk(s.start) >= minMin);

  return filtered.slice(0, limit);
}

export function filterSlotsByTimeRange(slots: SlotCandidate[], timeRange: string): SlotCandidate[] {
  if (!Array.isArray(slots) || slots.length === 0) return [];

  const tr = String(timeRange || "").trim().toUpperCase();
  ...
  const eh = Number(m[3]);
  const em = Number(m[4]);
  if ([sh, sm, eh, em].some((x) => Number.isNaN(x))) return slots;
  ...
  return slots.filter((s) => {
    ...
    return st >= startMin && st < endMin;
  });
}

export function pickBestSlot(slots: SlotCandidate[], timeRange: string): SlotCandidate | null {
