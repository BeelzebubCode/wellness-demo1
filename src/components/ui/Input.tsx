...
const Input = forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    // ✅ โหมด bare: ไม่มี wrapper/label/hint/error
    if (variant === "bare") {
      return (
        <input
          ref={ref}
    ...
    return (
      <div className="w-full space-y-2">
        {label && (
          <label
            htmlFor={inputId}
            className="block text-base lg:text-sm font-bold text-gray-700"
          >
            {label}
          </label>
        )}
        ...
        <div className="relative">
          ...
          <input
            ...
            className={cn(
              ...
              // ✅ แก้ typo: focus:ring- → focus:ring-2 (หรือ ring-1 แล้วแต่ชอบ)
        ...
        {error && (
          <p className="text-sm lg:text-base text-red-600 mt-1 font-medium">
            ⚠️ {error}
          </p>
        )}
        {hint && !error && (
          <p className="text-sm lg:text-base text-gray-500 mt-1">{hint}</p>
        )}
      </div>
    );
  },
