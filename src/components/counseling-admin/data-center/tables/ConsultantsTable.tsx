...
import type { ConsultantItem, PaginatedResponse, DataCenterFilter } from '@/features/data-center/types';
...
export default function ConsultantsTable({ filters }: Props) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (filters.activeQueueMin) params.set('activeQueueMin', String(filters.activeQueueMin));
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
                      ...
                      <td className="px-6 py-4 text-center">
                        <span
                          className={`font-medium ${
                            item.activeQueueCount > 0 ? 'text-amber-600' : 'text-gray-400'
                          }`}
                        >
      ...
      <ConsultantDetailModal consultantId={selectedId} onClose={() => setSelectedId(null)} />
    </div>
  );
}